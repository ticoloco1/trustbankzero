...
import { Gem, Play, Lock, Eye, RefreshCw, ShoppingCart, TrendingUp } from "lucide-react";
...
const NftFlipCard = ({ video, canPlay, isPlaying, onPlay, onBuy, soldOut, themeAccent = "#a855f7" }: NftFlipCardProps) => {
  const [flipped, setFlipped] = useState(false);

  const viewsRemaining = canPlay
    ? "∞" // will show actual remaining if we have purchase data
    : video.nft_max_views;
      ...
      <div className={`relative transition-transform duration-500 [transform-style:preserve-3d] ${flipped ? "[transform:rotateY(180deg)]" : ""}`}>
        ...
        <div className="[backface-visibility:hidden] bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden">
          <div className="relative aspect-video bg-black/20">
            {isPlaying && canPlay ? (
              <iframe src={`https://www.youtube.com/embed/${video.youtube_video_id}?autoplay=1`} allow="autoplay; encrypted-media" allowFullScreen className="w-full h-full" />
            ) : (
              <>
                {video.preview_url && !canPlay ? (
                  ...
                  <img src={video.thumbnail_url || `https://img.youtube.com/vi/${video.youtube_video_id}/hqdefault.jpg`} alt={video.title}
